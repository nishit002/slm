...
def format_formula_for_pdf(formula_text):
    ...
    def convert_superscript(match):
        try:
            return ''.join([superscript_map[int(d)] for d in match.group(1)])
        except:
            return match.group(0)
    
    def convert_subscript(match):
        try:
            return ''.join([subscript_map[int(d)] for d in match.group(1)])
        except:
            return match.group(0)
    
    formula_text = re.sub(r'\^(\d+)', convert_superscript, formula_text)
    formula_text = re.sub(r'_(\d+)', convert_subscript, formula_text)
    ...
    if completed < total:
        ...
        if not st.session_state.get('paused', False):
                ...
                if content and len(content.strip()) > 100:
                    ...
                    with col2:
                        if st.button("⏭️ Skip", use_container_width=True, key="skip_gen_btn"):
                            st.session_state.content[section_key] = f"[Content for {section_key}]"
                            st.rerun()
        else:
            ...
            with col1:
                if st.button("▶️ Resume", type="primary", use_container_width=True, key="resume_gen_btn"):
                    ...
                    st.rerun()
            with col2:
                if st.button("⏭️ Skip", use_container_width=True, key="skip_paused_btn"):
        ...
        with col2:
            if st.button("🔄 Regenerate All", use_container_width=True, key="regen_all_btn"):
                if st.checkbox("⚠️ Delete all?", key="regen_confirm"):
...
def show_image_management_page():
    ...
    with col1:
        if st.button("← Back", use_container_width=True, key="back_content_img_btn"):
            st.session_state.step = "content_generation"
            st.rerun()
    ...
    with col2:
        ...
        if total_count == 0:
            st.info("💡 No images needed")
        elif uploaded_count < total_count:
            st.warning(f"⚠️ {uploaded_count}/{total_count} uploaded")
        else:
            st.success(f"✅ All {total_count} uploaded!")
...
def show_compilation_page():
    ...
    if total_images > 0:
        if uploaded_count < total_images:
            st.warning(f"⚠️ {total_images - uploaded_count} image(s) missing")
        else:
            st.success(f"✅ All {total_images} images ready!")
   
    col1, col2, col3 = st.columns(3)
   
    with col1:
        if st.button("← Back", use_container_width=True, key="back_img_btn"):
            st.session_state.step = "image_management"
            st.rerun()
   
    with col2:
        if total_images > 0:
            if st.button("🖼️ Edit Images", use_container_width=True, key="edit_img_btn"):
                st.session_state.step = "image_management"
                st.rerun()
    ...
    if compile_button:
        ...
        if pdf_buffer:
            ...
            st.success("🎉 Your curriculum is ready!")
...
def main():
    ...
    with st.sidebar:
        ...
        st.divider()
        st.caption("💡 Auto-saved in session")
